...
async function generateIOHoroscope(sign, astroData) {
  ...
  const translateForConstellations = "Перевод для созвездий. Aries: Овен, Taurus: Телец, Gemini: Близнецы, Cancer: Рак, Leo: Лев, Virgo: Дева, Libra: Весы, Scorpius: Скорпион, Sagittarius: Стрелец, Capricornus: Козерог, Aquarius: Водолей, Pisces: Рыбы, Auriga: Возничий, Ophiuchus: Змееносец, Cetus: Кит, Orion: Орион, Sextans: Секстант.";

  const rulingPlanet = "Правящие планеты знаков зодиака. Овен: Марс, Телец: Венера, Близнецы: Меркурий, Рак: Луна, Лев: Солнце, Дева: Меркурий, Весы: Венера, Скорпион: Плутон, Марс, Стрелец: Юпитер, Козерог: Сатурн, Водолей: Уран, Сатурн, Рыбы: Нептун, Юпитер";
  
  const prompt = `
    Составь гороскоп для ${sign}.
    ...
    3. Учёт астрологии:
      - Упоминай небесные тела, если они находятся в знаке ${sign}, напиши какое влияние на знак ${sign} они окажут.
      - Упомяни правящую планету знака ${sign} (${rulingPlanet}), в каком созвездии она находится и как влияет на ${sign}.
    4. Запрещено:
      - Писать дату в гороскопе.
